...
import { CarouselFloorCard } from './CarouselFloorCard';
import { CarouselRoomCard } from './CarouselRoomCard';

export function CarouselFloorSection({ andar, salas }: FloorSectionProps) {
  // Dividir salas em duas colunas sequenciais
  const metade = Math.ceil(salas.length / 2);
  const colunaEsquerda = salas.slice(0, metade);
  const colunaDireita = salas.slice(metade);

  return (
    <div className="space-y-2">
      {/* Card do Andar */}
      <CarouselFloorCard andar={andar} />
      
      {/* Grid de Salas - duas colunas sequenciais */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-7xl mx-auto px-2 relative">
        {/* Coluna Esquerda */}
        <div className="space-y-2 relative">
          {colunaEsquerda.map((sala) => (
            <CarouselRoomCard key={sala.id} sala={sala} />
          ))}
        </div>
